...
{
    //Converts domain names into numerical IP addresses via Domain Name Service
    host = (struct hostent *)gethostbyname(host_name);

    //If host does not have anything, then quit the program.
    if (host == NULL) 
	{
    ...
    remoteAddr.sin_port = htons(5001); //Port 5001 is assigned
	...
	{
        perror("Failure to setup socket");
        exit(1);
    }
}
...
void server() 
{
	...
	{
        perror("Failure to bind server address to the socket");
        exit(1);
    }

    //Wait for client to request a file, once you receive something from the client, print out
    //its contents.
    int rb;
...
void client() 
{
    ...
    while ((rb = recvfrom(sock, rbuf, 1024, 0, (struct sockaddr *)&remoteAddr, &addrLen)) > 0)
	{
		...
		{
            puts("Remote server could not open the file.");
            exit(1);
        }
    }
...
int main() 
{
    while (1) 
    {
		...
		{
            puts("Error choosing number, please try a differnet number.");
        }
    }
    return 0;
}